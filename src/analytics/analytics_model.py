...
def create_analytics_schema():
    ...
    try:
        cursor.execute("CREATE SCHEMA IF NOT EXISTS ANALYTICS_MART")
        print("✓ Created ANALYTICS_MART schema")
    finally:
        cursor.close()
        conn.close()
...
def create_fact_tables():
    ...
    try:
        ...
        cursor.execute("""
            CREATE OR REPLACE TABLE ANALYTICS_MART.FACT_BOOKINGS AS
            SELECT
                t.TICKET_ID,
                t.USER_ID,
                t.SEAT_ID,
            ...
            LEFT JOIN STAGING.SEATS s ON t.SEAT_ID = s.SEAT_ID
        """)
        cursor.execute("SELECT COUNT(*) FROM ANALYTICS_MART.FACT_BOOKINGS")
        ...
        cursor.execute("""
            CREATE OR REPLACE TABLE ANALYTICS_MART.FACT_PAYMENTS AS
            SELECT
                p.PAYMENT_ID,
                p.USER_ID,
                p.TOTAL_PRICE,
        ...
        """)
        cursor.execute("SELECT COUNT(*) FROM ANALYTICS_MART.FACT_PAYMENTS")
        count = cursor.fetchone()[0]
        ...
        cursor.execute("""
            CREATE OR REPLACE TABLE ANALYTICS_MART.FACT_SEAT_SNAPSHOT AS
            SELECT
                s.INSTANCE_ID,
                s.CLASS,
            ...
            FROM STAGING.SEATS s
            GROUP BY s.INSTANCE_ID, s.CLASS
        """)
        cursor.execute("SELECT COUNT(*) FROM ANALYTICS_MART.FACT_SEAT_SNAPSHOT")
        count = cursor.fetchone()[0]
...
def create_dimension_tables():
    ...
    try:
        ...
        cursor.execute("""
            CREATE OR REPLACE TABLE ANALYTICS_MART.DIM_USERS AS
            SELECT
                u.USER_ID,
                u.NAME,
                u.EMAIL,
            ...
            LEFT JOIN STAGING.TICKETS t ON u.USER_ID = t.USER_ID 
                AND t.STATUS IN ('confirmed', 'pending')
            GROUP BY u.USER_ID, u.NAME, u.EMAIL, u.CREATED_AT
        """)
        cursor.execute("SELECT COUNT(*) FROM ANALYTICS_MART.DIM_USERS")
        count = cursor.fetchone()[0]
        ...
        cursor.execute("""
            CREATE OR REPLACE TABLE ANALYTICS_MART.DIM_SEAT_CLASSES AS
            SELECT DISTINCT
                CLASS as CLASS_NAME,
                CASE
            ...
            FROM STAGING.SEATS
        """)
        cursor.execute("SELECT COUNT(*) FROM ANALYTICS_MART.DIM_SEAT_CLASSES")
        count = cursor.fetchone()[0]
...
def create_analytical_views():
    ...
    try:
        ...
        cursor.execute("""
            CREATE OR REPLACE VIEW ANALYTICS_MART.VW_REVENUE_SUMMARY AS
            SELECT
                DATE_TRUNC('day', p.PURCHASED_DATE) as REVENUE_DATE,
                COUNT(DISTINCT p.PAYMENT_ID) as PAYMENT_COUNT,
                SUM(p.TOTAL_PRICE) as TOTAL_REVENUE,
                AVG(p.TOTAL_PRICE) as AVG_PAYMENT,
                COUNT(DISTINCT p.USER_ID) as UNIQUE_CUSTOMERS
            FROM ANALYTICS_MART.FACT_PAYMENTS p
            WHERE p.PAYMENT_STATUS = 'completed'
            GROUP BY DATE_TRUNC('day', p.PURCHASED_DATE)
        ...
        # Occupancy Trends
        cursor.execute("""
            CREATE OR REPLACE VIEW ANALYTICS_MART.VW_OCCUPANCY_BY_CLASS AS
            SELECT
                CLASS as SEAT_CLASS,
                TOTAL_SEATS,
                OCCUPIED_SEATS,
                AVAILABLE_SEATS,
                OCCUPANCY_RATE_PCT
            FROM ANALYTICS_MART.FACT_SEAT_SNAPSHOT
        ...
        # Customer Metrics
        cursor.execute("""
            CREATE OR REPLACE VIEW ANALYTICS_MART.VW_CUSTOMER_METRICS AS
            SELECT
                USER_ID,
                NAME,
                EMAIL,
        ...
        # Booking Funnel
        cursor.execute("""
            CREATE OR REPLACE VIEW ANALYTICS_MART.VW_BOOKING_FUNNEL AS
            SELECT
            ...
            SELECT
                'Confirmed',
                COUNT(*),
                ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM ANALYTICS_MART.FACT_BOOKINGS), 2)
            FROM ANALYTICS_MART.FACT_BOOKINGS
            ...
            SELECT
                ...
                COUNT(*),
                ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM ANALYTICS_MART.FACT_BOOKINGS), 2)
            FROM ANALYTICS_MART.FACT_PAYMENTS
