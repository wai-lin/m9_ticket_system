...
async def run_pipelined_rps_test(r: redis.Redis, total_count: int, chunk_size: int = 1000):
    ...
    data_list = [{"id": f"p_{i}", "u": "1", "s": "ok", "p": "450"} for i in range(total_count)]
...
async def main():
    # Pipelining doesn't need many connections, just one fast one.
    r = redis.from_url(REDIS_URL, decode_responses=True)

    try:
        # Test with 30,000 records to hit the 10k target
...
# FINISHED: 30000 operations in 5.52s
# IMEASURED THROUGHPUT: 5430.00 RPS