...
def report_revenue_summary():
    ...
    results = execute_query_dict("""
        SELECT * FROM ANALYTICS_MART.VW_REVENUE_SUMMARY LIMIT 10
    """)
...
def report_occupancy_metrics():
    ...
    results = execute_query_dict("""
        SELECT * FROM ANALYTICS_MART.VW_OCCUPANCY_BY_CLASS
    """)
...
def report_customer_metrics():
    ...
    results = execute_query_dict("""
        SELECT * FROM ANALYTICS_MART.VW_CUSTOMER_METRICS LIMIT 10
    """)
    ...
    if results:
        ...
        for segment, data in sorted(segments.items()):
            print(f"   {segment}: {data['count']} customers, ${data['revenue']:,.2f} revenue")
...
def report_booking_funnel():
    ...
    results = execute_query_dict("""
        SELECT * FROM ANALYTICS_MART.VW_BOOKING_FUNNEL ORDER BY PCT DESC
    """)
...
def get_kpi_summary():
    ...
    """)
    total_rev = float(result[0]['TOTAL_REVENUE']) if result and result[0]['TOTAL_REVENUE'] else 0
    print(f"💵 Total Revenue: ${total_rev:,.2f}")
    
    # Total Bookings
    result = execute_query_dict("SELECT COUNT(*) as COUNT FROM ANALYTICS_MART.FACT_BOOKINGS")
    ...
    # Conversion Rate
    conversion = (confirmed / total_bookings * 100) if total_bookings > 0 else 0
    print(f"📈 Conversion Rate: {conversion:.1f}%")
    
    # Total Customers
    result = execute_query_dict("SELECT COUNT(*) as COUNT FROM ANALYTICS_MART.DIM_USERS")
    ...
    """)
    avg_occ = float(result[0]['AVG_OCC']) if result and result[0]['AVG_OCC'] else 0
